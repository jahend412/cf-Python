    ...
    if session.query(Recipe).count() == 0:
        ...
        try:
            ingredient_searched_nber = input(
                "\nEnter the number corresponding to the ingredient you want to select from the above list. You can enter muliple recipes using a space between them.")
...
def delete_recipe():
    if session.query(Recipe).count() == 0:
        ...
        print("\nYou are about to delete the following recipe: ")
    ...
    if session.query(Recipe).count() == 0:
        ...
        if recipe_id_for_edit not in recipes_id_list:
            ...
            column_for_update = int(input(
                "\nEnter the data you want to update among 1. name, 2. cooking time and 3. ingredients: (Select the number for which one you want to edit"))
